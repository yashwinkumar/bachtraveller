...
$adult_num=$value['data']['adult_num'];
$child_num=$value['data']['child_num'];
            ...
            for($k=1;$k<=$room_num;$k++){
                ...
                <tr id="" class="room-item" data-room-num="<?php echo esc_attr($k) ?>">
                    ...
                    <td class="room_num_adult" style="width:12%;">
                        <select name="room_data[<?php echo esc_attr( $k) ?>][adult_num]" class="room_num_adults_input" >
                            <?php
                            $max=st()->get_option('hotel_max_adult',14);


                            for($i=1;$i<=$max;$i++){
                                echo "<option ".selected($i,$adult_num)." value='$i'>{$i}</option>";
                            }?>
                        </select>
                    </td>
                    ...
                    <td class="room_num_children" style="width:10%;">
                        <select name="room_data[<?php echo esc_attr( $k) ?>][child_num]" class="room_num_children_input">
                            ...
                            for($i=0;$i<=$max;$i++){
                                echo "<option value='$i' ".selected($i,$child_num).">{$i}</option>";
                            }?>
                        </select>
                    </td>
                    <td class=" ">
